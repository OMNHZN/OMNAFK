...
const MAX_PERSISTED_GAMES: usize = 200;
const MAX_LEARNED_PROFILES: usize = 100;
...
fn save_to_path(persisted: &PersistedStats, path: &Path) -> io::Result<()> {
    ...
    prune_persisted(&mut persisted);
    let json = serde_json::to_vec_pretty(&persisted)
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error.to_string()))?;
    crate::persist::atomic_write(path, &json)
}

fn prune_persisted(persisted: &mut PersistedStats) {
    while persisted.per_game.len() > MAX_PERSISTED_GAMES {
        let Some(key) = persisted
            .per_game
            .iter()
            .min_by_key(|(_, totals)| totals.kept.saturating_add(totals.actions))
            .map(|(identity, _)| identity.clone())
        else {
            break;
        };
        persisted.per_game.remove(&key);
    }

    while persisted.learned.len() > MAX_LEARNED_PROFILES {
        let Some(key) = persisted.learned.keys().next().cloned() else {
            break;
        };
        persisted.learned.remove(&key);
    }
}
