...
use crate::{
    config::{AppConfig, TargetAction, TargetProfile, DEFAULT_GITHUB_REPO},
    updates,
};
...
pub struct GameEntry {
    ...
    pub action_confidence: Option<f32>,
    pub monitor_confidence: Option<f32>,
    pub reports: u32,
...
pub fn spawn_sync_loop(community: SharedCommunity, engine: crate::engine::SharedEngine) {
    thread::spawn(move || loop {
        ...
        if result.is_err() {
            community.write().last_error =
                Some("Community sync recovered after an internal error.".to_string());
        }
...
fn refresh_manifest(community: &SharedCommunity, repo: &str) -> Result<(), String> {
    ...
    if !repo.eq_ignore_ascii_case(DEFAULT_GITHUB_REPO) {
        return Err("Community intelligence only uses the official OMNAFK manifest.".to_string());
    }
...
fn flush_contributions(
    ...
    if !trusted_ingest_url(&url) {
        return Err("Community upload endpoint is not trusted.".to_string());
    }
...
fn trusted_ingest_url(url: &str) -> bool {
...
pub fn snapshot_for_exe(
    ...
    Some(CommunityGameSnapshot {
        ...
        monitor_confidence: entry.monitor_confidence,
        ...
        top_keys: entry.top_keys.clone(),
        monitor_note: entry.monitor_note.clone(),
        applied,
    })
}
...
pub fn apply_game_profile(config: &mut AppConfig, exe: &str, wclass: &str, entry: &GameEntry) {
    ...
    set_game_profile(config, exe, wclass, entry);
}

pub fn set_game_profile(config: &mut AppConfig, exe: &str, wclass: &str, entry: &GameEntry) {
    ...
    profile.send_without_focus = entry.send_without_focus;
    profile.auto_fallback = entry.auto_fallback;
...
pub fn record_feedback(exe: &str, feedback: &str) {
    let feedback = match feedback {
        "worked" | "wrong_game" | "wrong_action" => feedback,
        _ => return,
    };
    let mut queue = load_queue();
    queue.reports.push(ContributionReport {
        ...
        monitor_ok: None,
        monitor_fail: None,
        stability_event: Some(format!("profile_feedback:{feedback}")),
    });
    let _ = save_queue(&queue);
}
...
mod tests {
    ...
    #[test]
    fn upload_endpoint_must_be_trusted() {
        assert!(trusted_ingest_url(
            "https://api.github.com/repos/OMNHZN/OMNAFK/dispatches"
        ));
        assert!(!trusted_ingest_url(
            "http://api.github.com/repos/OMNHZN/OMNAFK/dispatches"
        ));
        assert!(!trusted_ingest_url("https://example.com/collect"));
    }
}
