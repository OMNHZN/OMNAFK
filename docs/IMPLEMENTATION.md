...
The current frontend lives in `src/`. Keep future UI changes aligned with the shipped app and website screenshots instead of carrying separate public mockup files.
...
tab mirrors reality within 5s; UI is aligned with the shipped app and current screenshots;
...
**Accept:** clean VM: install → tray appears → first-run walkthrough (§7.4) → game detected
